...
// Karate uses node of green led for its white led.

#define LOG_TAG "LightService.karate"
...
static void handleKarateBacklight(const LightState& state) {
    uint32_t brightness = state.color & 0xFF;
    set(LCD_LED BRIGHTNESS, brightness);
}

static void handleKarateNotification(const LightState& state) {
...
static std::map<Type, std::function<void(const LightState&)>> lights = {
    {Type::BACKLIGHT, handleKarateBacklight},
    {Type::NOTIFICATIONS, handleKarateNotification},
    {Type::BATTERY, handleKarateNotification},
    {Type::ATTENTION, handleKarateNotification},
};
